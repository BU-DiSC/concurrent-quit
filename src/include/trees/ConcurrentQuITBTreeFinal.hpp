...
class BTree {
   public:
    ...
    /*
        Function to update fast-path metadata upon leaf insert
        Requires:
            (1) leaf to be locked
    ...
    */
    uint16_t determine_split_pos(node_t &leaf, uint16_t index, bool &fp_move) {
        ...
        auto expected_prev_metadata = fp_prev_metadata.load();
        if (leaf.info->id == fp_metadata.fp_id) {
            ...
            if (expected_prev_metadata.fp_prev_id == INVALID_NODE_ID ||
                expected_prev_metadata.fp_prev_size < IQR_SIZE_THRESH) {
                ...
                size_t max_distance = IKR::upper_bound(
                    dist(fp_metadata.fp_min,
                         expected_prev_metadata.fp_prev_min),
                    expected_prev_metadata.fp_prev_size, fp_metadata.fp_size);
            ...
            if (leaf.info->id == fp_metadata.fp_id) {
                ...
                if (expected_prev_metadata.fp_prev_id == INVALID_NODE_ID ||
                    ...
                    size_t max_distance = IKR::upper_bound(
                        dist(fp_metadata.fp_min,
                             expected_prev_metadata.fp_prev_min),
                        expected_prev_metadata.fp_prev_size,
                        fp_metadata.fp_size);
                    uint16_t outlier_pos =
                        leaf.value_slot2(fp_metadata.fp_min + max_distance);
                    if (outlier_pos <= SPLIT_LEAF_POS) {
   ...
   public:
    ...
    // Pessimistic insert used when the fast-path is full or when we want to
    // treat the operation like a generic top insert. Mirrors the control flow
    // used inline in tree_insert(): find leaf with path, attempt direct insert,
    // otherwise split and propagate up. The `fast` flag indicates whether
    // fast-path-related metadata updates are allowed inside leaf/split helpers
    // (e.g., when operating on FP or its predecessor) and should be passed in
    // by the caller according to context.
    void insert_pessimistic(const key_type &key, const value_type &value,
                            bool fast) {
        path_t path;
        node_t leaf;
        key_type leaf_max{};
        // Find target leaf and hold locks along the path.
        find_leaf_exclusive(leaf, path, key, leaf_max);

        // Determine whether this operation should be treated as operating on
        // the fast-path (either FP itself or its predecessor). This can
        // happen due to races: caller may have decided not to treat this as
        // fast, but by the time we find the leaf the FP may have moved.
        ...
        if (leaf_insert(leaf, index, key, value, treat_as_fast)) {
            // If we inserted into the fast-path, update its size (fp_mutex
            // is held either by caller or by local_fp_lock when required).
            if (leaf.info->id == fp_metadata.fp_id) {
                ++fp_metadata.fp_size;
            }
            for (const auto &parent_id : path) {
        ...
        // Otherwise, perform split. `split_insert` expects fp_mutex to be
        // held by the caller when `treat_as_fast == true` — we ensured that
        // above by acquiring `local_fp_lock` if necessary.
