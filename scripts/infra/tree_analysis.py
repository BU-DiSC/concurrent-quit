...
class TreeAnalysisRegex: 
    def __init__(self) -> None: 
        ...
        self.fast_inserts_regex = re.compile(r"\[[0-9 :.-]+\] \[.*?\] \[info\] fast inserts: (\d+)", flags)
        self.redistribute_regex = re.compile(r"\[[0-9 :.-]+\] \[.*?\] \[info\] redistribute: (\d+)", flags)
        self.soft_resets_regex = re.compile(r"\[[0-9 :.-]+\] \[.*?\] \[info\] soft resets: (\d+)", flags)
        self.hard_resets_regex = re.compile(r"\[[0-9 :.-]+\] \[.*?\] \[info\] hard resets: (\d+)", flags)
